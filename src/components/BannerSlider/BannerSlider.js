...
export default function BannerSlider() {
    ...
    return(
        <Box sx={{overflow: "hidden"}}>
            <Box sx={{position:"relative", height: "fit-content", minHeight:"450px"}}>
                ...
                <ImageList 
                    ...
                    cols={bannerSlides.length} gap={0}
                >
                    {bannerSlides.map((item, index) => (
                        <ImageListItem key={`banner-${index}`} sx={{width: windowWidth, height: "fit-content"}} id={`banner-${index}`}>
                            <Link to={routeBuilder(ROUTE_LIST.SINGLE_PRODUCT, item.productId)}>
